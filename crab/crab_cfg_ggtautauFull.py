...
config.JobType.scriptExe = 'crab_script_ggtautauFull.sh'
#config.JobType.scriptArgs= "foo"
# hadd nano will not be needed once nano tools are in cmssw
config.JobType.inputFiles = ['crab_script_ggtautauFull.py', '../scripts/haddnano.py', 'keep_and_drop.txt']
...
config.Data.outLFNDirBase = '/store/user/legianni/skimNano-HggHtautauselection' # cannot getUsernameFromSiteDB
config.Data.publication = False
config.Data.outputDatasetTag = 'skimNano-HggHtautauselection'+version
...
for sample in allsamples:
    ...
    config.General.requestName = 'skimNano-HggHtautauselection'+sample[0:30].replace(".","")+"--"+str(counter)
